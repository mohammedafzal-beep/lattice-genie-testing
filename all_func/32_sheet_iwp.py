...
import os
import threading
def Sheet_IWP(C, t,resolution = 200 , folder='all_files'):
    def IWP_function(x, y, z, scale=1, c=1.0):
    ...
    def generate_solid_volume(size, resolution, scale, c, t):
        ...
        values = IWP_function(x, y, z, scale, c)
        values1 = -IWP_function(x, y, z, scale, c-t)
        # # Modify values outside the cube to ensure one space is solid
        values[np.cos(2 * x * scale) + np.cos(2 * y * scale) + np.cos(2 * z * scale) - 2 * np.cos(x * scale) * np.cos(y * scale) - 2 * np.cos(y * scale) * np.cos(z * scale) - 2 * np.cos(z * scale) * np.cos(x * scale)<=c-t] =\
            values1[np.cos(2 * x * scale) + np.cos(2 * y * scale) + np.cos(2 * z * scale) - 2 * np.cos(x * scale) * np.cos(y * scale) - 2 * np.cos(y * scale) * np.cos(z * scale) - 2 * np.cos(z * scale) * np.cos(x * scale)<=c-t]
        values[(np.cos(2 * x * scale) + np.cos(2 * y * scale) + np.cos(2 * z * scale) - 2 * np.cos(x * scale) * np.cos(y * scale) - 2 * np.cos(y * scale) * np.cos(z * scale) - 2 * np.cos(z * scale) * np.cos(x * scale)>c-t) &
            (np.cos(2 * x * scale) + np.cos(2 * y * scale) + np.cos(2 * z * scale) - 2 * np.cos(x * scale) * np.cos(y * scale) - 2 * np.cos(y * scale) * np.cos(z * scale) - 2 * np.cos(z * scale) * np.cos(x * scale)<=c-t/2)] \
            = values1[(np.cos(2 * x * scale) + np.cos(2 * y * scale) + np.cos(2 * z * scale) - 2 * np.cos(x * scale) * np.cos(y * scale) - 2 * np.cos(y * scale) * np.cos(z * scale) - 2 * np.cos(z * scale) * np.cos(x * scale)>c-t) &
            (np.cos(2 * x * scale) + np.cos(2 * y * scale) + np.cos(2 * z * scale) - 2 * np.cos(x * scale) * np.cos(y * scale) - 2 * np.cos(y * scale) * np.cos(z * scale) - 2 * np.cos(z * scale) * np.cos(x * scale)<=c-t/2)] 
    ...
    def create_stl_from_mesh(verts, faces, folder, filename="Sheet_IWP.stl"):
    ...
    filename = f"32Sheet_IWP_{C:.1f}_{resolution}.stl"
